...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const articlePages: MetadataRoute.Sitemap = articles.map((article) => ({
    url: `${baseUrl}/${article.slug.startsWith('problems/') || article.slug.startsWith('cost-guides/') || article.slug.startsWith('reviews/') || article.slug.startsWith('comparisons/') ? article.slug : `articles/${article.slug}`}`,
